...
cli = FlaskGroup(create_app=create_app) #used to create commands for managing the application.

user_json = [       
	{                                   #These dictionaries contain seeded data of the database. They store the customer details.
...
product_json = [                      #these dictionaries contain data about the products for sale in the ecommerce website.
	{
...
def recreate_db():    #This recreate_db function resets the data base by deleting the old data n recreating a new data base.
	db.drop_all()
	db.create_all()
	db.session.commit()

def seeder():    #This function re enters dummy data into the newly created data base.
...
if __name__ == '__main__':
	cli()
  	...
  	#The recreate_db() and seeder() are then used in the above function called rsd() which also has a decorator @cli.command .
